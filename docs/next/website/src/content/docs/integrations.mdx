---
title: Integrations
description: Install Herdr integrations for Pi, OMP, Claude Code, Codex, GitHub Copilot CLI, Devin CLI, Droid, Kimi Code CLI, OpenCode, Kilo Code CLI, Hermes Agent, Qoder CLI, Qwen Code, Cursor Agent CLI, MastraCode, Antigravity CLI, Grok CLI, and Kiro CLI.
---
...
| Session identity | Claude Code, Codex, GitHub Copilot CLI, Devin CLI, Droid, Qoder CLI, Qwen Code, Cursor Agent CLI, Hermes Agent, Antigravity CLI, Grok CLI, Kiro CLI | The integration reports native session references for restore. State still comes from Herdr's screen manifest detection. |
...
Install the Kiro CLI hook (requires Kiro CLI 3.0+):

```bash
herdr integration install kiro
```

The hook reports session identity through Kiro's `SessionStart` and `Stop` hooks while Kiro CLI runs inside a Herdr pane. Kiro state comes from Herdr's screen manifest detection.

Herdr uses `~/.kiro`. The Kiro config directory must already exist. Kiro merges every `hooks/*.json` file in that directory, so install writes a self-contained `hooks/herdr-agent-state.json` with the Herdr `SessionStart` and `Stop` entries next to `hooks/herdr-agent-state.sh` (`hooks/herdr-agent-state.ps1` on Windows), and never edits other hook files. Uninstall removes exactly those two Herdr-owned files.

After Kiro emits a session start event, Herdr can use the reported session id to resume the pane with `kiro-cli chat --resume-id <id>`.
